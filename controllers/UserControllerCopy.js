...
const verifyEmailController = async (req, res) => {
  ...
  if (
    userStoreLink[userIdString] &&
    userStoreLink[userIdString].verificationToken === token
  ) {
    if (Date.now() <= userStoreLink[userIdString].expiresAt) {
      delete userStoreLink[userIdString];
      const user = await UserModel.findById(userId);
    ...
    res
      .status(400)
      .json({ message: "Invalid verification link or user does not exist" });
  }
};
...
const sendCutomMailController = async (req, res) => {
  ...
  const mailOptions = {
    // Nodemailer "from" format: "Name" <email@domain>
    from: `"${process.env.WEBSITE_NAME}" <${process.env.NODEMAILER_SENDER_EMAIL}>`,
    to: email,
    subject,
    html: `${content}`,
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Custom mail sent!", messageId: info.messageId });
  } catch (error) {
    // Optional: don’t leak internals in production
    return res.status(500).json({ message: "Error sending custom mail" });
  }
};
