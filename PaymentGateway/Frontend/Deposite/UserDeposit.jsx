...
const handleCreditCardClick = async () => {
  ...
  try {
    if (!loggedUser?.email || !formData.depositAmount || !formData.accountNumber) {
      toast.error("⚠️ Enter all required details.");
      return;
    }

    const res = await fetch("http://localhost:4000/api/auth/paygate/create-wallet", {
      method: "POST",
